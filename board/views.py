...
def contact(request):
    if request.method == 'POST':
        ...
        # 발신자주소, 수신자주소, 메시지
        send_mail('codingmaster97@gmail.com', email,title, comment)
        return render(request, 'contact_success.html')
    return render(request, 'contact.html')

def send_mail(from_email, to_email,title, msg):
    smtp = smtplib.SMTP_SSL('smtp.gmail.com', 465) # SMTP 설정
    smtp.login(from_email, 'nznlmgmtaiomafux') # 인증정보 설정
    msg = MIMEText(msg)
